...
namespace Xmax.SDK
{
    ...
    public sealed class RealtimeMediaStream
    {
        ...
        /// <summary>Push an external frame into a local stream on the Unity main thread.</summary>
        public void PushVideoFrame(XmaxVideoFrame frame)
        {
            if (_push == null) throw new XmaxException(XmaxErrorCode.InvalidConfiguration, "Only a local external stream accepts input frames.");
            _push(frame);
        }

        internal RealtimeMediaStream(RealtimeStreamId id, RealtimeVideoTrack videoTrack,
            RealtimeVideoFormat? videoFormat = null, Action<XmaxVideoFrame> push = null)
        {
            Id = id;
