...
namespace Xmax.SDK
{
    ...
    public sealed class XmaxVideoFrame
    {
            ...
            XmaxVideoRotation rotation = XmaxVideoRotation.Rotation0)
        {
            ...
            ValidatePlane(y, strideY * height, nameof(y));
            ValidatePlane(u, strideU * (height / 2), nameof(u));
            ValidatePlane(v, strideV * (height / 2), nameof(v));
        ...
        internal void Validate()
        {
            ...
            if (PixelFormat == XmaxVideoPixelFormat.I420)
            {
                ...
                ValidatePlane(Planes[0], Strides[0] * Height, "Y");
                ValidatePlane(Planes[1], Strides[1] * (Height / 2), "U");
                ValidatePlane(Planes[2], Strides[2] * (Height / 2), "V");
            }
            else
            {
                ...
                ValidatePlane(Planes[0], Strides[0] * Height, "RGBA/BGRA");
            }
        }
            ...
            XmaxVideoRotation rotation)
        {
            ...
            if (stride < width * 4)
            {
                throw new XmaxException(XmaxErrorCode.InvalidConfiguration, "RGBA/BGRA stride is smaller than width * 4.");
            }
            ValidatePlane(data, stride * height, nameof(data));
            ValidateRotation(rotation);
            return new XmaxVideoFrame(
                format,
                width,
        ...
        private static void ValidatePlane(byte[] data, int minimumLength, string name)
        {
