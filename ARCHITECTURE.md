...
状态与帧事件中的宿主异常不影响清理，开启业务日志后记录错误类型。回调触发状态变化或帧失效后，剩余过期通知会被抑制。异步致命错误清理后通过 `ErrorOccurred` 通知；API 调用失败由 Task 抛出；关闭 Session 的网络失败作为 `CleanupWarning` 报告。终态通过 `RealtimeState.Reason` 携带进入原因：主动停止或取消为 `Normal`，失败为 `Failure(error)`，与 `ErrorOccurred` 或 Task 抛出共享同一错误实例；进行中的状态原因为空，新操作开始时清空。对齐 iOS 时剔除了眼镜平台不存在的方向变化原因。
